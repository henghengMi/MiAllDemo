...
@interface MiPOPCasualController ()<POPAnimationDelegate>

@end

@implementation MiPOPCasualController
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    for (int i=1; i < 4; i++) {
        for (int j=1; j<4; j++) {
            CGRect frame = CGRectMake( 50 + (diameter + 15) * (j-1), 80 + (diameter + 30) *(i-1), diameter, diameter);
...
- (void) addBckgColorBasicChangeAnimationToView:(UIView *)view {
    ...
    anim.toValue = (__bridge id)([UIColor yellowGreenColor].CGColor);
...
- (void) addBckgColorSpringChangeAnimationToView:(UIView *)view {
    ...
    anim.toValue =  (__bridge id)([UIColor waveColor].CGColor);
...
- (void) animateRoundToProgressIndicatorToView:(UIView *) view {
    ...
    POPSpringAnimation *anim = [POPSpringAnimation animationWithPropertyNamed:kPOPLayerBounds];
    anim.toValue = [NSValue valueWithCGRect:CGRectInset(view.frame, -60, 25)];
    anim.springBounciness = 8;
    anim.delegate = self;
    [view.layer pop_addAnimation:anim forKey:@"popBounds"];
